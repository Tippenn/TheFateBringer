...
public class BulkScript : GiantBaseScript
{
    ...
    public IEnumerator AttackTarget(Direction targetDirection)
    {
        ...
        audioManager.PlaySFX(audioManager.bulk);
        yield return StartCoroutine(MoveTowardsDirection(targetDirection, () => { }));
        StopAttackAnimation();
        Grid<TileMap.TilemapObject> grid = tilemapTesting.GetGrid();
        Vector2Int unitPosition = grid.GetXY(GetPosition());
        ...
        if (targetDirection == Direction.Left)
        {
            for(int x  = unitPosition.x-1; x >= 0; x--)
            {
                ...
                if (unitGridCombat)
                {
                    ...
                    if (unitGridCombat.isDead != true)
                    {
                        tilemapObject = grid.GetGridObject(x - 1, unitPosition.y);
                        if (tilemapObject.isBlocking != true)
                        {
                            yield return StartCoroutine(unitGridCombat.MoveTo(new Vector2Int(x - 1, unitPosition.y), () => { }));
                        }
                    }
                    break;
                }
        ...
        else if(targetDirection == Direction.Up)
        {
            for (int y = unitPosition.y+1; y < grid.GetHeight(); y++)
            {
                ...
                if (unitGridCombat)
                {
                    ...
                    if (unitGridCombat.isDead != true)
                    {
                        tilemapObject = grid.GetGridObject(unitPosition.x, y+1);
                        if (tilemapObject.isBlocking != true)
                        {
                            yield return StartCoroutine(unitGridCombat.MoveTo(new Vector2Int(unitPosition.x, y + 1), () => { }));
                        }
                    }
                    break;
                }
